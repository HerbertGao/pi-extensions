...
export function getAvailableTypesIn(
  registry: Map<string, AgentConfig>,
): string[] {
...
export function resolveEnabledTypeIn(
  ...
  return key !== undefined && registry.get(key)?.enabled !== false
    ? key
    : undefined
}
...
export function resolveSpawnTypeIn(
  ...
  if (configured !== undefined) {
    ...
    if (
      ...
      registry.get(fallbackKey)?.enabled === false
    ) {
      return {
        ok: false,
        message:
