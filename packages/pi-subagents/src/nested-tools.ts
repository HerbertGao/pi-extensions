...
export function createNestedSubagentTools(
  ...
  const loadRegistry = () =>
    buildAgentRegistry(loadCustomAgents(context.configCwd))
  const allowedTypesIn = (
    registry: Map<string, AgentConfig>,
  ): Set<string> | undefined =>
    context.allowedSubagents === "all"
      ? undefined
      : new Set(
          context.allowedSubagents.map(
            (name) => resolveTypeIn(registry, name) ?? name,
          ),
        )
  ...
  const agentTool = defineTool({
    ...
    parameters: Type.Object({
      prompt: Type.String({
        description: "Self-contained task for the nested agent.",
      }),
    ...
    execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
      if (params.resume) {
        ...
        return resumed
          ...
          : textResult(
              `Failed to resume nested agent "${params.resume}".`,
              true,
            )
      }
      ...
      const rootSessionId = context.manager.getRecord(
      ...
      const options: NestedSpawnOptions = {
        ...
        onAssistantUsage: (usage) => {
          for (
            let id: string | undefined = context.parentAgentId;
            id !== undefined;
          ) {
      ...
      options.onSessionCreated = (session) => {
        const rec =
          childId === undefined ? undefined : context.manager.getRecord(childId)
  ...
  const resultTool = defineTool({
    ...
    description:
      "Check or wait for a background nested agent owned by this parent.",
    parameters: Type.Object({
  ...
  const steerTool = defineTool({
    ...
    description:
      "Send guidance to a running nested agent owned by this parent.",
    parameters: Type.Object({
      agent_id: Type.String(),
      message: Type.String(),
    }),
