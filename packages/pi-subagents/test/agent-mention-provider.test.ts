...
function builtIn() {
  return {
    ...
    applyCompletion: vi
      .fn()
      .mockReturnValue({ lines: ["applied"], cursorLine: 0, cursorCol: 7 }),
...
const suggest = (
  provider: ReturnType<typeof createMentionProvider>,
  line: string,
) =>
...
const agentRows = (result: Awaited<ReturnType<typeof suggest>>) =>
  (result?.items ?? []).filter(
    (item) => !(FILE_SUGGESTIONS.items as unknown[]).includes(item),
  )
...
describe("agent suggestions", () => {
      ...
      items: [
        {
          value: "@explore",
          label: "@explore",
          description: "send message · running · find flaky tests",
        },
  ...
  it("offers every agent on a bare @, and still offers files", async () => {
    ...
    const provider = createMentionProvider(
      ...
      () =>
        mentionRoster(
          managerWith(
            record({ handle: "explore" }),
            record({ handle: "plan", startedAt: 2000 }),
          ),
          [],
        ),
      () => true,
  ...
  it("still offers agents when the wrapped provider throws", async () => {
    ...
    const current = builtIn()
    current.getSuggestions.mockRejectedValue(
      new Error("inner provider exploded"),
    )
    const provider = createMentionProvider(
      current,
      () => mentionRoster(managerWith(record({ handle: "explore" })), []),
  ...
  it("puts steerable agents first, then earliest-launched", async () => {
    ...
    expect(agentRows(result).map((i) => i.value)).toEqual([
      "@explore-2",
      "@explore-3",
      "@explore",
    ])
  ...
  it("matches case-insensitively", async () => {
    ...
    expect(
      agentRows(await suggest(provider, "@EX")).map((i) => i.value),
    ).toEqual(["@explore"])
  })
...
describe("agents that have never run", () => {
  const TYPES = [
    {
      name: "Explore",
      description: "Fast codebase exploration. Read-only, medium breadth.",
    },
  ...
  it("lets a live agent own its handle instead of listing the type twice", async () => {
    ...
    expect(agentRows(result)[0].description).toBe(
  ...
  it("still offers the type once its only instance has finished, as a resume", async () => {
    const provider = createMentionProvider(
      ...
      () =>
        mentionRoster(
          managerWith(record({ handle: "explore", status: "completed" })),
          TYPES,
        ),
      () => true,
    )
  ...
  it("lists live agents before startable types", async () => {
    ...
    expect(agentRows(await suggest(provider, "@")).map((i) => i.value)).toEqual(
...
describe("composing with another extension's provider", () => {
  ...
  function hashWrapper(current: any) {
    return {
      ...
      getSuggestions: vi.fn(
        async (lines: string[], line: number, col: number, opts: any) =>
          /(^|\s)#\w*$/.test(lines[line].slice(0, col))
            ? { items: [{ value: "#general", label: "#general" }], prefix: "#" }
            : current.getSuggestions(lines, line, col, opts),
      ),
      applyCompletion: vi.fn((...args: any[]) =>
        (current.applyCompletion as any)(...args),
      ),
    }
  }
  ...
  it("delegates to a provider registered before us", async () => {
    ...
    expect(
      (await suggest(provider, "#gen"))?.items.map((i) => i.value),
    ).toEqual(["#general"])
  ...
  it("is still reachable through a provider registered after us", async () => {
    ...
    expect(
      agentRows(
        await outer.getSuggestions(["@ex"], 0, 3, {
          signal: new AbortController().signal,
        }),
      ).map((i) => i.value),
    ).toEqual(["@explore"])
    expect(
      (
        await outer.getSuggestions(["#g"], 0, 2, {
          signal: new AbortController().signal,
        })
...
describe("against pi's real provider", () => {
  ...
  it("inserts a FILE-shaped row from OUR prefix, character for character", async () => {
    ...
    const suggestions = (await suggest(p, "@ex"))!
    const fileRow = {
      value: "@examples/agent-tool-description.md",
      label: "agent-tool-description.md",
    }

    expect(
      p.applyCompletion(["@ex"], 0, 3, fileRow, suggestions.prefix).lines,
    ).toEqual(["@examples/agent-tool-description.md "])
    expect(
      p.applyCompletion(["ask @ex now"], 0, 7, fileRow, suggestions.prefix)
        .lines,
...
describe("named agents and evicted ones", () => {
    ...
    expect(agentRows(await suggest(provider, "@"))).toEqual([
      {
        value: "@explore",
        label: "@explore",
        description: "resume · Explore · audit the RPC path",
      },
    ])
  })
  ...
  it("puts live agents ahead of resumable ones", async () => {
    ...
    const manager = {
      listAgents: () => [
        record({ handle: "plan", type: "Plan", status: "running" }),
      ],
  ...
  it("keeps an aliased tombstone's type handle reserved too", () => {
    ...
    const roster = mentionRoster(
      managerWithTombstones(
        tombstone({ handle: "explore", alias: "auth-audit" }),
      ),
      [{ name: "Explore", description: "search" }],
    )
  ...
  it("does not offer a startable type whose handle a tombstone still holds", () => {
    ...
    const roster = mentionRoster(managerWithTombstones(tombstone()), [
      { name: "Explore", description: "search" },
    ])
...
describe("rows carry the display name, not the raw type", () => {
  ...
  it("falls back to the raw type when no resolver is supplied", async () => {
    ...
    expect((await suggest(provider, "@"))!.items[0].description).toContain(
      "· Explore ·",
    )
  })
})
