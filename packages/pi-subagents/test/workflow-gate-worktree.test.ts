 ...
 */
function makePi(
  gate: (command: string) => ExecResult | Promise<ExecResult> = () =>
    execOk("3 passing"),
) {
...
const execOk = (stdout = ""): ExecResult => ({
  stdout,
  stderr: "",
  code: 0,
  killed: false,
})
const execFail = (stderr: string): ExecResult => ({
  stdout: "",
  stderr,
  code: 1,
  killed: false,
...
function childWrites(fallback: string) {
  vi.mocked(runAgent).mockImplementation(
    async (_ctx: any, _type: any, _prompt: any, opts: any) => {
      writeFileSync(
        join(opts.cwd ?? fallback, CHILD_FILE),
...
const HEAD =
  'export const meta = { name: "probe", description: "gate over a worktree" };\n'
...
describe("gate on an isolated child", () => {
  ...
  it("fails the agent with the gate's output, and still cleans the worktree up", async () => {
    const { pi, gateRuns } = makePi(() =>
      execFail("FAIL src/auth.test.ts\n1 failing"),
    )
    const host = createWorkflowHost({ pi, ctx: ctx({ cwd: repo }), manager })
  ...
  it("names the command when a failing gate in a worktree says nothing", async () => {
    ...
    expect(agentEntries(result.progress).at(-1)?.error).toBe(
  ...
  it("counts a killed gate as failed even though pi.exec reports exit code 0", async () => {
    ...
    const result = await host.spawnAgent(
      spawnRequest({ isolation: "worktree", gate: "sleep 999" }),
    )
...
describe("an isolated child with no gate", () => {
  ...
  it("is untouched: nothing runs, and the work still lands on a branch", async () => {
    ...
    const result = await host.spawnAgent(
      spawnRequest({ isolation: "worktree" }),
    )
