...
function unphasedEntry(
  index: number,
  partial: Partial<WorkflowAgentEntry> = {},
): WorkflowAgentEntry {
  return {
    ...agentEntry({ index, ...partial }),
...
describe("displayState", () => {
  const cases: [string, Partial<WorkflowAgentEntry>, boolean, string][] = [
    ...
    [
      "queued then started",
      { state: "start", queuedAt: 5, startedAt: 6 },
      true,
      "running",
    ],
  ...
  for (const [name, partial, active, expected] of cases) {
    it(name, () => {
      expect(displayState(agentEntry({ index: 0, ...partial }), active)).toBe(
        expected,
      )
    })
  }
  ...
  it("reports done even after the run stops", () => {
    expect(displayState(agentEntry({ index: 0, state: "done" }), false)).toBe(
      "done",
    )
  })
})
...
describe("buildPhaseGroups", () => {
  ...
  describe("merging with meta.phases", () => {
    ...
    it("matches when the declared title is a prefix of the observed one", () => {
      const groups = buildPhaseGroups(
        [
          { type: "workflow_phase", index: 0, title: "Review changed files" },
          agentEntry({ index: 0 }),
        ],
        [{ title: "Review" }],
      )
...
describe("stats", () => {
  ...
  it("does not count a queued-but-unstarted agent as started", () => {
    const result = stats([
      agentEntry({ index: 0, state: "start", queuedAt: 5 }),
    ])
...
describe("sizeWarning", () => {
  ...
  it("does not fire exactly at the cap", () => {
    expect(
      sizeWarning({ scheduledAgents: 25, startedAgents: 25, totalTokens: 10 }),
    ).toBeUndefined()
  })
...
describe("gerund", () => {
  ...
  it.each([["setup"], ["cleanup"], ["Running"]])("leaves %s alone", (input) => {
    expect(gerund(input)).toBe(input)
  })

  it.each([["QA"], ["a-very-long-phase-title"], ["Phase 1"]])(
    "leaves the non-word %s alone",
    (input) => {
      expect(gerund(input)).toBe(input)
    },
  )
...
describe("footerPhaseLabel", () => {
  it("shows position for a single active phase", () => {
    expect(
      footerPhaseLabel({ titles: ["Scan"], positionStart: 1, totalPhases: 3 }),
    ).toBe("Scanning (1/3)")
  })
  ...
  it("truncates a long title", () => {
    ...
    const label = footerPhaseLabel({
      titles: ["Comprehensive review"],
      positionStart: 1,
      totalPhases: 2,
    })
  ...
  it("returns empty when no phase is active", () => {
    expect(
      footerPhaseLabel({ titles: [], positionStart: 0, totalPhases: 0 }),
    ).toBe("")
  })
})
