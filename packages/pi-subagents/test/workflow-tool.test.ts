...
import {
  ...
  makePi,
  textOf,
} from "./helpers/boot-extension.js"
...
function stubManager(
  settle: (
    ...
    prompt: string,
    options: any,
  ) => Promise<AgentRecord> | AgentRecord = () => record(),
): StubManager {
  const abort = vi.fn()
  ...
  return {
    manager: { spawnAndWait, abort, resume } as unknown as AgentManager,
    spawnAndWait,
    abort,
    resume,
  }
...
const request = (
  overrides: Partial<WorkflowSpawnRequest> = {},
...
describe("createWorkflowHost — spawn mapping", () => {
  ...
  it("spawns through the manager and maps a completed record onto the script's result", async () => {
    const stub = stubManager(() =>
      record({
        result: "the answer",
        toolUses: 3,
        lifetimeUsage: { input: 100, output: 20, cacheWrite: 5 },
      }),
    )
  ...
  it("routes an unknown agent type through the same dispatch the Agent tool uses", async () => {
    ...
    const fellBack = await host.spawnAgent(
      request({ agentType: "no-such-agent" }),
    )
    ...
    try {
      ...
      const strictHost = createWorkflowHost({
        pi: {} as any,
        ctx: ctx(),
        manager: strict.manager,
      })
      const rejected = await strictHost.spawnAgent(
        request({ agentType: "no-such-agent" }),
      )
  ...
  it("forwards a compiled schema and prefers the structured payload", async () => {
    ...
    const result = await host.spawnAgent(
      request({ schema: compilation.compiled }),
    )

    expect(stub.spawnAndWait.mock.calls[0][4].structuredOutput).toBe(
  ...
  it("maps an errored record onto an error, and a stopped one onto skipped", async () => {
    ...
    const failed = await createWorkflowHost({
      pi: {} as any,
      ctx: ctx(),
      manager: failing.manager,
    }).spawnAgent(request())
    const skipped = await createWorkflowHost({
      pi: {} as any,
      ctx: ctx(),
      manager: stopped.manager,
    }).spawnAgent(request())
...
describe("createWorkflowHost — scopeModels", () => {
  ...
  it("refuses a model the script named that is out of scope, and fails only that agent", async () => {
    ...
    const blocked = await host.spawnAgent(
      request({ model: "anthropic/blocked-model" }),
    )
    ...
    const inScope = await host.spawnAgent(
      request({ model: "anthropic/allowed-model" }),
    )
  ...
  it("toasts a repeated warning once, not once per child of a fan-out", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const spawned = await host.spawnAgent(
        request({ agentId: `wf-agent-${i}`, agentType: "pinned" }),
      )
  ...
  it("leaves every spawn alone when scopeModels is off — the default", async () => {
    ...
    const named = await host.spawnAgent(
      request({ model: "anthropic/blocked-model" }),
    )
    const pinnedAgent = await host.spawnAgent(
      request({ agentId: "wf-agent-1", agentType: "pinned" }),
    )
...
describe("createWorkflowHost — worktree cwd propagation", () => {
  ...
  it("reports no cwd once the worktree has been torn down", async () => {
    ...
    const stub = stubManager(() =>
      record({
        worktree: { path: gone, branch: "b", baseSha: "sha", workPath: gone },
      }),
    )
    const host = createWorkflowHost({
      pi: {} as any,
...
describe("createWorkflowHost — abort, resume and gate", () => {
  ...
  it("asks the manager for a pre-cleanup hook only when the agent is gated", async () => {
    ...
    await host.spawnAgent(request({ isolation: "worktree" }))
    expect(
      stub.spawnAndWait.mock.calls[0][4].onBeforeWorktreeCleanup,
    ).toBeUndefined()

    await host.spawnAgent(request({ isolation: "worktree", gate: "npm test" }))
    expect(
      stub.spawnAndWait.mock.calls[1][4].onBeforeWorktreeCleanup,
  ...
  it("runs a gate through pi.exec in the child's own tree", async () => {
    ...
    const gate = await host.runGate?.("npm test", {
      agentId: "wf-agent-0",
      cwd: "/worktree",
    })
  ...
  it("fails a gate on a non-zero exit and surfaces its output", async () => {
    const exec = vi.fn(
      async (
        ...
        _args: string[],
        _options: { cwd?: string; timeout?: number },
      ) => execResult({ code: 1, stderr: "1 failing" }),
    )
    const host = createWorkflowHost({
      pi: { exec } as any,
      ctx: ctx(),
    ...
    expect(await host.runGate?.("npm test", { agentId: "wf-agent-0" })).toEqual(
...
const inlineScript =
  'export const meta = { name: "from-inline", description: "inline source" };\n'
const fileScript =
  'export const meta = { name: "from-file", description: "file source" };\n'
...
describe("SubagentWorkflow tool — script vs scriptPath vs name", () => {
  ...
  it("runs the inline script when only `script` is given, and persists it", async () => {
    ...
    expect(
      scriptLine,
      "the persisted script path is what makes iteration edit-then-rerun",
    ).toBeTruthy()
  ...
  it("asks for one of the three when none is given", async () => {
    const result = await tools
      .get("SubagentWorkflow")
      .execute("tc-5", {}, undefined, undefined, workflowCtx())
  ...
  it("runs a saved workflow by name from .pi/workflows", async () => {
    ...
    writeFileSync(
      join(hermetic.dir, ".pi", "workflows", "nightly.js"),
      fileScript,
    )
  ...
  it("falls back to the user's agent dir, and lets the project shadow it", async () => {
    ...
    mkdirSync(join(hermetic.dir, ".pi", "workflows"), { recursive: true })
    writeFileSync(
      join(hermetic.dir, ".pi", "workflows", "shared.js"),
      inlineScript,
    )

    const fromProject = await tools
      .get("SubagentWorkflow")
      .execute(
        "tc-name-7",
        { name: "shared" },
  ...
  it("refuses a file in the folder that is not a workflow", async () => {
    ...
    mkdirSync(join(hermetic.dir, ".pi", "workflows"), { recursive: true })
    writeFileSync(
      join(hermetic.dir, ".pi", "workflows", "utils.js"),
  ...
  it("refuses a run id this session never issued", async () => {
    ...
    expect(textOf(result)).toMatch(
      /No workflow run "wf_deadbeef1234" in this session/,
    )
    expect(textOf(result)).not.toMatch(/started/)
  })
  ...
  it("accepts a settled run and says plainly that there was nothing to replay", async () => {
    ...
    expect(textOf(result)).toMatch(
      new RegExp(`Nothing to replay from ${runId}`),
    )
  ...
  it("names the workflow on the call line", () => {
    const line =
      tools
        .get("SubagentWorkflow")
        .renderCall({ script: inlineScript }, plainTheme, { isPartial: false })
        .text ?? ""
  ...
  it("actually runs the script in the background and notifies through the agent channel", async () => {
    ...
    const result = await tools
      .get("SubagentWorkflow")
      .execute("tc-run", { script }, undefined, undefined, workflowCtx())
    ...
    await vi.waitFor(
      () =>
        expect(
          booted.pi.sendMessage.mock.calls.some((c: any[]) =>
            String(c[0]?.content).includes(taskId!),
          ),
        ).toBe(true),
      { timeout: 10_000 },
    )

    const sent = booted.pi.sendMessage.mock.calls.find((c: any[]) =>
      String(c[0]?.content).includes(taskId!),
    )!
  ...
  async function awaitNotification(taskId: string) {
    ...
    return booted.pi.sendMessage.mock.calls.find((c: any[]) =>
      String(c[0]?.content).includes(taskId),
    )!
  }

  const startedTaskId = (result: unknown) =>
    /Task ID: (\S+)/.exec(textOf(result))![1]
...
describe("--subagents-workflow-file", () => {
  ...
  it("registers the fleet row as soon as a run starts, not when it settles", async () => {
    ...
    expect(
      keys,
      "the run has to claim its row before its first agent starts",
    ).toContain("fleet")
  })
  ...
  it("captures the UI at session_start, before any tool has executed", () => {
    ...
    expect(
      context.ui.onTerminalInput,
      "the fleet list only hooks input once it has a UI",
    ).toHaveBeenCalled()
  })
  ...
  it("registers the flag at activation but does not read it there", () => {
    ...
    expect(booted.registeredFlags.get(WORKFLOW_FILE_FLAG)).toMatchObject({
      type: "string",
    })
  ...
  it("explains the `=` form when the flag arrives bare as boolean true", async () => {
    ...
    expect(
      notices.some((m: string) => m.includes(`--${WORKFLOW_FILE_FLAG}=<path>`)),
    ).toBe(true)
    expect(booted.pi.appendEntry).not.toHaveBeenCalledWith(
      WORKFLOW_ENTRY_TYPE,
      expect.anything(),
    )
  })
  ...
  it("reports a script it cannot read without starting a run", async () => {
    const booted = makePi({
      [WORKFLOW_FILE_FLAG]: join(hermetic.dir, "absent.js"),
    })
    subagentsExtension(booted.pi)
    const context = uiCtx()
    ...
    expect(
      context.ui.notify.mock.calls.some((c: any[]) =>
        /Could not read/.test(String(c[0])),
      ),
    ).toBe(true)
    expect(booted.pi.appendEntry).not.toHaveBeenCalledWith(
      WORKFLOW_ENTRY_TYPE,
  ...
  it("renders the finished run as a session entry and hands it to the model as next-turn context", async () => {
    ...
    const sent = booted.pi.sendMessage.mock.calls.find(
      (c: any[]) => c[0]?.customType === "workflow-result",
    )
  ...
  it("renders the persisted entry through the shared workflow card layout", async () => {
    ...
    expect(
      renderer,
      "a custom entry with no renderer is silently dropped by the host",
    ).toBeTruthy()
...
describe("workflowsEnabled — the master switch", () => {
  ...
  it("stays off when the setting says so explicitly", () => {
    ...
    expect(
      boot({ workflowsEnabled: false }).tools.has("SubagentWorkflow"),
    ).toBe(false)
  })

  it("registers the tool once the setting turns it on", () => {
    expect(boot({ workflowsEnabled: true }).tools.has("SubagentWorkflow")).toBe(
      true,
    )
  })
...
describe("collisions with another extension", () => {
  ...
  const boot = (
    settings: Record<string, unknown> = { workflowsEnabled: true },
  ) => {
    hermetic = hermeticDir({ settings })
    const booted = makePi()
    subagentsExtension(booted.pi)
  ...
  const warnings = (context: any): string[] =>
    context.ui.notify.mock.calls
      .filter((c: any[]) => c[1] === "warning")
  ...
  it("warns when another extension already owns the tool name", async () => {
    ...
    expect(
      warnings(context).some((m) =>
        /already registers a "SubagentWorkflow" tool/.test(m),
      ),
    ).toBe(true)
  ...
  it("withdraws its tool when another extension provides a `Workflow` tool", async () => {
    ...
    expect(
      warnings(context).some((m) =>
        /already provides a "Workflow" tool/.test(m),
      ),
    ).toBe(true)
    expect(warnings(context).some((m) => /other-ext/.test(m))).toBe(true)
  })
  ...
  it("stands down without withdrawing when the foreign tool took our own name", async () => {
    ...
    expect(
      warnings(context).some((m) =>
        /already provides a "SubagentWorkflow" tool/.test(m),
      ),
    ).toBe(true)
  })
  ...
  it("keeps ours when the setting pins workflows on", async () => {
    ...
    expect(booted.pi.getActiveTools()).toContain("SubagentWorkflow")
    expect(
      warnings(context).filter((m) => /disabled for this session/.test(m)),
    ).toEqual([])
  })
  ...
  const savedSettings = () =>
    JSON.parse(
      readFileSync(join(hermetic.dir, ".pi", "subagents.json"), "utf-8"),
    )
