...
function agentEntry(
  partial: Partial<WorkflowAgentEntry> & { index: number },
): WorkflowAgentEntry {
  return {
    type: "workflow_agent",
...
const leftRows = (lines: string[]): string[] =>
  trimTrailing(cells(lines).map((row) => row.left))
const rightRows = (lines: string[]): string[] =>
  trimTrailing(cells(lines).map((row) => row.right))
...
function section(lines: string[], heading: string): string[] {
  ...
  if (start < 0)
    throw new Error(`no ${heading} section in:\n${lines.join("\n")}`)
  ...
  return (end < 0 ? rest : rest.slice(0, end)).map((row) =>
    row.replace(/^ {3}/, "  "),
  )
}
...
describe("dialog glyph mapping", () => {
  ...
  it("maps every one of the seven display states to its glyph and colour", () => {
    expect(dialogRowGlyph("done", UNICODE_DIALOG_GLYPHS)).toEqual({
      text: "✔",
      color: "success",
    })
    expect(dialogRowGlyph("failed", UNICODE_DIALOG_GLYPHS)).toEqual({
      text: "✘",
      color: "error",
    })
    expect(dialogRowGlyph("skipped", UNICODE_DIALOG_GLYPHS)).toEqual({
      text: "✘",
      color: "dim",
    })
    expect(dialogRowGlyph("blocked", UNICODE_DIALOG_GLYPHS)).toEqual({
      text: "✘",
      color: "warning",
    })
    expect(dialogRowGlyph("queued", UNICODE_DIALOG_GLYPHS)).toEqual({
      text: "◌",
      color: "dim",
    })
    expect(dialogRowGlyph("interrupted", UNICODE_DIALOG_GLYPHS)).toEqual({
      text: "◌",
      color: "dim",
    })
    expect(dialogRowGlyph("running", UNICODE_DIALOG_GLYPHS, 3)).toEqual({
      text: SPINNER[3],
      color: "dim",
    })
  })
  ...
  it("renders blocked distinctly from skipped despite sharing the cross", () => {
    ...
    expect(lines.find((l) => l.includes("blocked"))).toContain(
      "<warning>✘</warning>",
    )
    expect(lines.find((l) => l.includes("failed"))).toContain(
      "<error>✘</error>",
    )
    expect(lines.find((l) => l.includes(">done"))).toContain(
      "<success>✔</success>",
    )
  })
...
describe("phases pane", () => {
  ...
  it("points at the selected phase and accents it, leaving the others alone", () => {
    const rows = leftRows(
      styled({ progress, meta, state: { selectedPhase: 1 } }),
    )
  ...
  it("swaps the agent list when the phase selection moves", () => {
    ...
    expect(
      rightRows(dialog({ progress, meta, state: { selectedPhase: 1 } }))[0],
    ).toContain(" b")
  })

  it("names the selected phase and its agent count on the right pane", () => {
    expect(paneTitles(dialog({ progress, meta })).left).toBe("Phases")
    expect(paneTitles(dialog({ progress, meta })).right).toBe(
      "Review · 1 agent",
    )
    expect(
      paneTitles(dialog({ progress, meta, state: { selectedPhase: 1 } })).right,
    ).toBe("Verify · 1 agent")
  })
})
...
describe("state filter", () => {
  ...
  it("reports a filter that matches nothing, and says so in the list", () => {
    ...
    expect(rightRows(lines).map((row) => row.trim())).toEqual([
      WORKFLOW_DIALOG_COPY.noAgents,
    ])
...
describe("sub-status annotations", () => {
  it("renders the retry reason and attempt number from the entry", () => {
    ...
    expect(
      subStatusAnnotations(
        agentEntry({ index: 0, lastAttemptReason: "user-retry" }),
        "running",
        START,
      ),
    ).toEqual(["user retry"])
    expect(
      subStatusAnnotations(
        agentEntry({ index: 0, lastAttemptReason: "stalled" }),
        "running",
        START,
      ),
    ).toEqual(["stalled"])
  ...
  it("marks a journal replay and an isolated child", () => {
    expect(
      subStatusAnnotations(
        agentEntry({ index: 0, cached: true, isolation: "worktree" }),
        "done",
        START,
      ),
  ...
  it("shows how long a queued agent has waited, and only while it is queued", () => {
    ...
    expect(subStatusAnnotations(entry, "queued", START + 8000)).toEqual([
      "waiting 8s",
    ])
  ...
  it("puts the annotations on the row ahead of the card's stat tail", () => {
    ...
    expect(rows[0].trim()).toBe(
      "❯ ◌ retry-me · throttled · attempt 2 · waiting 8s",
    )
  })
})
...
describe("per-agent detail", () => {
  ...
  it("collapses a long prompt behind an `expand` affordance and counts its lines", () => {
    ...
    expect(
      rightRows(expanded)
        .map(bare)
        .find((l) => l.includes("Prompt")),
    ).toContain("Prompt · 9 lines · ⏎ collapse")
  ...
  it("does not offer expand for a prompt that already fits, and counts one line as one", () => {
    const heading = (lines: string[]) =>
      rightRows(lines)
        .map((row) => bare(row).trim())
        .find((row) => row.startsWith("Prompt"))
    const two = detail({
      progress: [
        agentEntry({ index: 0, state: "done", promptPreview: "one\ntwo" }),
      ],
    })
    expect(heading(two)).toBe("Prompt · 2 lines")
    const one = detail({
      progress: [
        agentEntry({ index: 0, state: "done", promptPreview: "solo" }),
      ],
    })
  ...
  it("uses Claude Code's copy for a prompt and activity that do not exist yet", () => {
    const lines = detail({
      progress: [agentEntry({ index: 0, state: "start", queuedAt: START })],
    })
    expect(section(lines, "Prompt")).toEqual([
      `  ${WORKFLOW_DIALOG_COPY.availableOnceStarted}`,
    ])
    expect(section(lines, "Activity")).toEqual([
  ...
  it("distinguishes no-tool-calls-yet from no-tool-calls-ever", () => {
    const running = detail({
      progress: [agentEntry({ index: 0, state: "progress", startedAt: START })],
    })
    expect(section(running, "Activity")).toEqual(["  No tool calls yet."])
    const finished = detail({
      progress: [agentEntry({ index: 0, state: "done" })],
    })
  ...
  it("heads Activity with the tool-call count and admits it has no transcript", () => {
    const heading = (lines: string[]) =>
      rightRows(lines)
        .map((row) => bare(row).trim())
        .find((row) => row.startsWith("Activity"))
    const lines = detail({
      progress: [agentEntry({ index: 0, state: "done", toolCalls: 3 })],
    })
    expect(heading(lines)).toBe("Activity · 3 tool calls")
    expect(section(lines, "Activity")).toEqual(["  Transcript not available."])
    const one = detail({
      progress: [agentEntry({ index: 0, state: "done", toolCalls: 1 })],
    })
  ...
  it("writes a different Outcome for every terminal state", () => {
    const outcome = (
      entry: Partial<WorkflowAgentEntry>,
      task?: WorkflowCardTask,
    ) =>
      section(
        detail({ progress: [agentEntry({ index: 0, ...entry })], task }),
        "Outcome",
      )

    expect(outcome({ state: "error", skipped: true })).toEqual([
      "  Skipped by user.",
    ])
    expect(outcome({ state: "error", error: "boom" })).toEqual(["  boom"])
    expect(outcome({ state: "error", blocked: true })).toEqual([
      `  ${WORKFLOW_DIALOG_COPY.noTranscript}`,
    ])
    expect(outcome({ state: "done", resultPreview: "shipped" })).toEqual([
      "  shipped",
    ])
    ...
    expect(
      outcome(
        { state: "progress", startedAt: START },
        { status: "killed", startTime: START },
      ),
    ).toEqual(["  The workflow stopped before this agent finished."])
  })
  ...
  it("omits the detail sections entirely when nothing is selected", () => {
    ...
    expect(leftRows(lines).map((row) => row.trim())).toEqual([
      WORKFLOW_DIALOG_COPY.noAgents,
    ])
  })
})
...
describe("keys", () => {
  ...
  const press = (data: string, state?: Partial<WorkflowDialogState>) => {
    ...
    return handleWorkflowDialogKey(
      data,
      full.state,
      resolveWorkflowDialog(full),
    )
  }
  ...
  it("moves the agent selection with j/k and clamps at both ends", () => {
    ...
    expect(
      press("j", { ...agents, selectedAgent: 1 })?.state.selectedAgent,
    ).toBe(1)
    expect(
      press("k", { ...agents, selectedAgent: 1 })?.state.selectedAgent,
    ).toBe(0)
    expect(
      press("k", { ...agents, selectedAgent: 0 })?.state.selectedAgent,
    ).toBe(0)
  })
  ...
  it("never runs the cursor off an empty list", () => {
    ...
    expect(
      handleWorkflowDialogKey("j", empty.state, view)?.state.selectedAgent,
    ).toBe(0)
  ...
  it("refuses to open a phase with nothing in it", () => {
    ...
    expect(
      handleWorkflowDialogKey("\r", empty.state, resolveWorkflowDialog(empty))
        ?.state.level,
    ).toBe("phases")
  })
  ...
  it("toggles the prompt expansion", () => {
    ...
    expect(press("e", { promptExpanded: true })?.state.promptExpanded).toBe(
      false,
    )
  ...
  it("raises the run-level actions, and pause flips to resume once paused", () => {
    ...
    expect(
      handleWorkflowDialogKey("p", paused.state, resolveWorkflowDialog(paused))
        ?.action,
    ).toEqual({
      kind: "resume",
    })
  ...
  const pressMixed = (data: string, selectedAgent: number) => {
    const full = input({
      progress: mixed,
      state: { level: "agent", selectedAgent },
    })
    return handleWorkflowDialogKey(
      data,
      full.state,
  ...
  it("uses the entry's index, not its row position", () => {
    ...
    const full = input({
      progress: across,
      state: { level: "agent", selectedPhase: 1, selectedAgent: 0 },
    })
    const view = resolveWorkflowDialog(full)

    expect(view.selectedEntry?.label).toBe("v0")
    expect(handleWorkflowDialogKey("s", full.state, view)?.action).toEqual({
      kind: "skip",
      index: 3,
    })
    expect(handleWorkflowDialogKey("r", full.state, view)?.action).toEqual({
      kind: "retry",
      index: 3,
    })
  })
  ...
  it("opens the selected agent's conversation on c, at either level", () => {
    ...
    expect(
      pressRecords("c", { level: "agent", selectedAgent: 1 })?.action,
    ).toEqual({
      kind: "open",
      recordId: "rec-1",
    })
    expect(
      pressRecords("c", { level: "phases", selectedAgent: 1 })?.action,
    ).toEqual({
      kind: "open",
      recordId: "rec-1",
    })
  ...
  it("opens a settled agent's conversation, unlike skip and retry", () => {
    ...
    expect(
      pressRecords("c", { level: "agent", selectedAgent: 2 })?.action,
    ).toEqual({
      kind: "open",
      recordId: "rec-2",
    })
  ...
  it("opens nothing for a row whose child has no record yet", () => {
    ...
    expect(
      pressRecords("c", { level: "agent", selectedAgent: 0 }),
    ).toBeUndefined()
  })
  ...
  it("refuses the run-level actions once the run has settled", () => {
    ...
    for (const status of ["completed", "failed", "killed"] as const) {
      const settled = input({
        progress,
        task: { status, startTime: START, endTime: START + 10 },
      })
      const view = resolveWorkflowDialog(settled)
...
describe("frame height", () => {
  ...
  const bodyHeight = (over: DialogOverrides): number =>
    cells(dialog(over)).length
  ...
  it("sizes the box to its content instead of a fixed height", () => {
    ...
    expect(bodyHeight({ progress: phased(3) })).toBeLessThan(
      DEFAULT_PANE_BODY_ROWS,
    )
  })
  ...
  it("caps the detail pane when a long prompt is expanded", () => {
    expect(bodyHeight({ progress: longPrompt, state: expanded })).toBe(
      DEFAULT_PANE_BODY_ROWS,
    )
  })
  ...
  it("honours an explicit cap from the caller", () => {
    expect(
      bodyHeight({ progress: longPrompt, state: expanded, bodyRows: 9 }),
    ).toBe(9)
...
describe("width", () => {
  const wide: WorkflowEntry[] = [
    {
      type: "workflow_phase",
      index: 0,
      title: "A phase title that runs on well past any sane terminal width",
    },
    agentEntry({
      index: 0,
      label:
        "an-extremely-long-agent-label-that-would-otherwise-wrap-the-whole-dialog",
      state: "done",
      ...
      promptPreview:
        "a prompt line long enough to overflow a narrow terminal all by itself",
      resultPreview:
        "an outcome line long enough to overflow a narrow terminal all by itself",
    }),
  ]
  ...
  it("reserves six columns of chrome from the terminal width", () => {
    ...
    expect(
      Math.max(...dialog({ progress: wide, width: 86 }).map(visibleWidth)),
    ).toBe(80)
  })
  ...
  it("never exceeds the content width", () => {
    for (const width of [4, 12, 20, 40, 80]) {
      ...
      for (const line of lines)
        expect(visibleWidth(line), `w=${width}`).toBeLessThanOrEqual(content)
    }
  })
...
describe("WorkflowDialog component", () => {
  ...
  function harness(from: () => ReturnType<typeof source> = source): {
  ...
  it("dispatches the injected actions and closes on escape", () => {
    ...
    expect(calls.filter((c) => c !== "render")).toEqual([
      "kill",
      "pause",
      "skip:7",
      "retry:7",
...
describe("key hints reflect the wired actions", () => {
  ...
  it("hides the conversation key when the caller did not wire it", () => {
    ...
    expect(
      agentHints({ progress: openable, available: { onOpenAgent: false } }),
    ).not.toContain("c convo")
  })
  ...
  const everyKey: WorkflowEntry[] = [
    {
      ...
      recordId: "rec-1",
      promptPreview: Array.from(
        { length: PROMPT_COLLAPSED_LINES + 2 },
        (_, i) => `line ${i}`,
      ).join("\n"),
    },
  ]
  ...
  it("drops the conversation hint before the way out on a narrow terminal", () => {
    ...
    const narrow =
      dialog({ progress: everyKey, width: 80, state: { level: "agent" } }).at(
        -1,
      ) ?? ""
  ...
  it("shows every hint once the terminal has room", () => {
    const wide =
      dialog({ progress: everyKey, width: 100, state: { level: "agent" } }).at(
        -1,
      ) ?? ""
    for (const key of [
      "↑↓ agent",
  ...
  it("hides stop when kill is not wired", () => {
    expect(
      hintLine({ progress: live, available: { onKill: false } }),
...
describe("component availability", () => {
  ...
  it("advertises the full set when every action is wired", () => {
    ...
    const hints = footer(instance)
    for (const key of ["s skip", "r retry", "p pause", "x stop"])
      expect(hints).toContain(key)
    instance.dispose()
  })
  ...
  it("advertises nothing actionable when no actions are wired", () => {
    ...
    const hints = footer(instance)
    for (const key of ["s skip", "r retry", "p pause", "x stop"])
      expect(hints).not.toContain(key)
    expect(hints).toContain("esc back")
    instance.dispose()
  })
})
