...
describe("disableInContent", () => {
  ...
  it("is idempotent when the key is already first", () => {
    expect(disableInContent(EXTENSION_WRITTEN_DISABLED).outcome).toBe(
      "already-disabled",
    )
  })

  it("is idempotent when the key is already present mid-block", () => {
    expect(disableInContent(HAND_AUTHORED_DISABLED).outcome).toBe(
      "already-disabled",
    )
  })
...
describe("parseAgentFrontmatter", () => {
  it("reads a BOM-prefixed file's fields instead of dropping them", () => {
    ...
    const { frontmatter, body } =
      parseAgentFrontmatter<Record<string, unknown>>(src)
  ...
  it("leaves a file without a BOM exactly as the parser reads it", () => {
    ...
    expect(parseAgentFrontmatter<Record<string, unknown>>(src)).toEqual(
      parseFrontmatter<Record<string, unknown>>(src),
    )
  })
})
  ...
  describe("locateAgentFile", () => {
    ...
    it("classifies a workspace and a personal source path", () => {
      ...
      expect(
        locateAgentFile(
          "code-reviewer",
          join(tmpDir, ".agents", "agents", "reviewer.md"),
          tmpDir,
        ),
      ...
      expect(
        locateAgentFile(
          "code-auditor",
          join(agentDir, "agents", "auditor.md"),
          tmpDir,
        ),
...
describe("buildNewAgentFile", () => {
  ...
  const parse = (content: string) =>
    parseFrontmatter<Record<string, unknown>>(content).frontmatter
  ...
  it("emits the fields the wizard collects, and omits the ones left on inherit", () => {
    const full = parse(
      buildNewAgentFile({ ...base, model: "anthropic/x", thinking: "high" }),
    )
  ...
  it("keeps the system prompt as the body", () => {
    ...
    expect(parseFrontmatter<Record<string, unknown>>(content).body).toBe(
      "Line 1.\n\nLine 2.",
    )
  })
})
