...
const HEAD =
  'export const meta = { name: "probe", description: "a test workflow" };\n'
...
function stubHost(
  ...
  ) => Promise<WorkflowSpawnResult> | WorkflowSpawnResult,
): Stub {
  ...
  return {
    ...
    host: {
      async spawnAgent(request) {
        ...
        return reply
          ? await reply(request)
          : { ok: true, text: `ok:${request.prompt}` }
      },
      abortAgent(agentId) {
...
function run(
  body: string,
  options: Omit<RunWorkflowOptions, "script">,
...
const agentEntries = (
  ...
  progress.filter(
    (entry): entry is WorkflowAgentEntry => entry.type === "workflow_agent",
  )
...
describe("script globals", () => {
  it("runs a script, returns its value and reports meta", async () => {
    ...
    const result = await run(
      'const answer = await agent("hello");\nreturn { answer };',
      { host },
    )
  ...
  it("records phases, logs and console output", async () => {
    ...
    expect(
      result.progress
        ...
        .map((e) => e.message),
    ).toEqual(["scanned 41 files", "a 1"])

    const byIndex = new Map(
      agentEntries(result.progress).map((e) => [e.index, e]),
    )
  ...
  it("maps a failed agent to null rather than throwing", async () => {
    const { host } = stubHost((request) =>
      request.prompt === "bad"
        ? { ok: false, error: "child exploded" }
        : { ok: true, text: "fine" },
    )
    const result = await run(
      'return [await agent("bad"), await agent("good")];',
      { host },
    )

    expect(result.status).toBe("completed")
    expect(result.value).toEqual([null, "fine"])
    const failed = agentEntries(result.progress).find(
      (e) => e.state === "error",
    )
...
describe("parallel", () => {
  ...
  it("rejects more items than the cap allows", async () => {
    const { host } = stubHost()
    const result = await run(
      "return await parallel(new Array(9).fill(async () => 1));",
      {
        host,
        itemCap: 8,
      },
...
describe("pipeline", () => {
  it("overlaps stages — item A reaches stage 2 before item B leaves stage 1", async () => {
    ...
    expect(value.order.indexOf("s2-in:A:0")).toBeLessThan(
      value.order.indexOf("s1-out:B"),
    )
  ...
  it("rejects more items than the cap allows", async () => {
    const { host } = stubHost()
    const result = await run(
      "return await pipeline(new Array(5).fill(1), async v => v);",
      {
        host,
        itemCap: 4,
      },
...
describe("semaphore", () => {
  ...
  it("queues agents it cannot start yet", async () => {
    ...
    const queuedOnly = agentEntries(result.progress).filter(
      (e) => e.queuedAt != null && e.startedAt == null,
    )
  ...
  it("makes Date.now, new Date() and Math.random throw with guidance", async () => {
    ...
    for (const message of value.messages) {
      expect(message).toContain(
        "unavailable in workflow scripts (breaks resume)",
      )
      expect(message).toContain(
...
describe("the JSON boundary", () => {
  ...
  it("rejects args that cannot cross, before the worker starts", async () => {
    const { host } = stubHost()
    await expect(
      run("return 1;", { host, args: { when: new Date(0) } }),
    ).rejects.toThrow(/across the workflow VM boundary/)
  })

  it("accepts plain JSON in both directions", () => {
    expect(() =>
      assertBoundarySafe({ a: [1, "two", null, { b: true }] }, "args"),
    ).not.toThrow()
...
describe("caps and validation", () => {
  ...
  it("rejects a script with control characters", async () => {
    const { host } = stubHost()
    await expect(
      runWorkflow({ script: `${HEAD}return "a\u0007b";`, host }),
  ...
  it("rejects a script over the size limit", async () => {
    ...
    await expect(runWorkflow({ script, host })).rejects.toThrow(
      /over the limit of 524288/,
    )
  })

  it("rejects a script with no meta block", async () => {
    const { host } = stubHost()
    await expect(runWorkflow({ script: "return 1;", host })).rejects.toThrow(
      /must begin with/,
    )
  })
...
describe("abort", () => {
    ...
    const promise = run(
      'await agent("hangs forever");\nreturn "unreachable";',
      {
        ...
        onProgress(entries) {
          if (
            entries.some(
              (e) => e.type === "workflow_agent" && e.startedAt != null,
            )
          )
            started()
        },
  ...
  describe("effective configuration reported mid-run", () => {
    const rowsFor = (
      ...
      seen
        .flat()
        .filter(
          (e): e is WorkflowAgentEntry =>
            e.type === "workflow_agent" && e.index === index,
        )
    ...
    it("replaces the requested model with the effective one, before the agent settles", async () => {
      ...
      const result = await run(
        'return await agent("go", { model: "haiku" });',
        {
          host,
          onProgress(entries) {
            seen.push([...entries])
    ...
    it("carries the request forward when it was not honoured (#182)", async () => {
      ...
      await run('return await agent("go", { effort: "max" });', {
        host,
        onProgress(e) {
          seen.push([...e])
        },
    ...
    it("leaves the row alone when the host reports nothing", async () => {
      ...
      await run('return await agent("go", { model: "haiku" });', {
        host,
        onProgress(e) {
          seen.push([...e])
        },
  ...
  it("settles a script that never yields", async () => {
    ...
    const promise = run('await agent("start");\nfor (;;) {}', {
      ...
      onProgress(entries) {
        if (
          entries.some((e) => e.type === "workflow_agent" && e.state === "done")
        )
          spinning()
      },
    })
  ...
  it("settles immediately when the signal is already aborted", async () => {
    const { host, calls } = stubHost()
    const result = await run('await agent("never");', {
      host,
...
describe("run control", () => {
  ...
  function controllableHost() {
    ...
    return {
      ...
      started: () => started,
      aborted: () => aborted,
      finish: (agentId: string, text: string) =>
        release.get(agentId)?.({ ok: true, text }),
    }
  }
  ...
  it("skips an agent held at a pause without waiting for the resume", async () => {
    ...
    await until(
      () => control?.skip(1) === true,
      "the second call to reach the gate",
    )
...
describe("Claude Code option compatibility", () => {
  it("names a misspelt option rather than ignoring it", async () => {
    const stub = stubHost()
    const result = await run(
      "return await agent('go', { agenttype: 'Explore' });",
      { host: stub.host },
    )
...
describe("the budget global", () => {
  ...
  it("runs Claude Code's static-scaling pattern verbatim", () => {
    return run(
      "return budget.total ? Math.floor(budget.total / 100_000) : 5;",
      {
        host: stubHost().host,
      },
    ).then((result) => {
...
describe("structured output", () => {
  ...
  it("fails the call when the host returns prose instead", async () => {
    ...
    expect(String(agentEntries(result.progress).at(-1)?.error)).toMatch(
      /not JSON|did not match/,
    )
  })
  ...
  it("rejects a schema that cannot be one, before spending a model call", async () => {
    for (const bad of ["5", "'x'", "[]", "{ type: 'array' }"]) {
      const stub = stubHost()
      const result = await run(
        `return await agent('go', { schema: ${bad} });`,
        { host: stub.host },
      )
  ...
  it("declines a replayed answer that no longer matches", async () => {
    ...
    const result = await run(
      `return (await agent('go', { schema: ${schemaLiteral} })).findings[0];`,
      {
        host: stub.host,
...
describe("nested workflow()", () => {
      ...
      host: {
        ...
        loadWorkflow(ref: { name?: string; scriptPath?: string }) {
          ...
          return script !== undefined
            ...
            : {
                ok: false as const,
                message: `No saved workflow named "${ref.name}".`,
              }
        },
  ...
  it("runs the child and returns its value", async () => {
    const stub = nestingHost({
      audit: child("audit", "return 'from the child';"),
    })
    const result = await run("return await workflow('audit');", {
      host: stub.host,
    })
  ...
  it("passes args and exposes the child's own meta", async () => {
    ...
    const result = await run("return await workflow('audit', { n: 7 });", {
      host: stub.host,
    })
  ...
  it("shares the parent's agent counter, so ids never collide", async () => {
    const stub = nestingHost({
      audit: child("audit", "await agent('child-a'); return 1;"),
    })
    const result = await run(
      "await agent('parent'); await workflow('audit'); return 1;",
      {
        host: stub.host,
      },
    )
  ...
  it("shares the parent's concurrency limit", async () => {
    ...
    let peak = 0
    const stub = nestingHost(
      { audit: child("audit", "await agent('child'); return 1;") },
      () => {
        live++
    ...
    await run(
      "await parallel([() => agent('a'), () => workflow('audit')]); return 1;",
      {
        host: stub.host,
        concurrency: 1,
      },
  ...
  it("files the child's agents under their own phase group", async () => {
    const stub = nestingHost({
      audit: child("audit", "await agent('scan'); return 1;"),
    })
    const result = await run(
      "phase('Review'); await agent('a'); await workflow('audit'); return 1;",
      {
        host: stub.host,
      },
    )
    ...
    const titles = result.progress
      .filter(
        (entry): entry is Extract<WorkflowEntry, { type: "workflow_phase" }> =>
  ...
  it("does not let the child's phase leak back to the parent", async () => {
    const stub = nestingHost({
      audit: child("audit", "phase('Scan'); await agent('c'); return 1;"),
    })
    const result = await run(
    ...
    const after = agentEntries(result.progress).find(
      (entry) => entry.label === "after",
    )
  ...
  it("keeps a child's phase from aliasing a parent phase of the same name", async () => {
    ...
    const result = await run(
      "phase('Scan'); await agent('p'); await workflow('audit'); return 1;",
      {
        host: stub.host,
      },
    )
  ...
  it("refuses to nest more than one level, by name", async () => {
    ...
    const result = await run(
      "try { await workflow('outer'); return 'no throw'; } " +
  ...
  it("journals the child's agents as the run's own", async () => {
    ...
    const replayed = await run(script, {
      host: replayHost.host,
      journal: { entries },
    })
