...
describe("BOM-prefixed agent files", () => {
  ...
  it("handles BOM + CRLF together, which is what a Windows editor writes", () => {
    ...
    expect(loadCustomAgents(tmpDir).get("审查员")?.description).toBe(
      "代码审查员",
    )
  ...
  it("disables and re-enables on disk, leaving the file byte-identical", () => {
    ...
    writeFileSync(
      path,
      disableInContent(readFileSync(path, "utf-8")).content,
      "utf-8",
    )
    ...
    writeFileSync(
      path,
      enableInContent(readFileSync(path, "utf-8")).content,
      "utf-8",
    )
