...
function runSpending(usage: {
  ...
  vi.mocked(runAgent).mockImplementation(
    async (_c: any, _t: any, _p: any, opts: any) => {
...
describe("reporting subagent usage back to the parent session", () => {
  ...
  it("reports what a background resume spends, on the next call", async () => {
    ...
    await spawn(tools, "tc-1")
    await flush()
    const id = pi.events.emit.mock.calls.find(
      (c: any[]) => c[0] === "subagents:completed",
    )?.[1]?.id

    vi.mocked(resumeAgent).mockImplementation(
      async (_session: any, _prompt: any, opts: any) => {
        ...
        if (!nested) {
          ...
          const childId = manager.spawn(pi, ctx(), "general-purpose", "sub", {
            ...
            onAssistantUsage: (u: any) =>
              addUsage(manager.getRecord(parentAgentId).lifetimeUsage, u),
          })
  ...
  describe("the lifecycle event payload", () => {
    ...
    async function completedPayload(pi: any) {
      await flush()
      const call = pi.events.emit.mock.calls.find(
        (c: any[]) => c[0] === "subagents:completed",
      )
      return call?.[1]
    }
    ...
    it("keeps `tokens` as the display total it has always been", async () => {
      ...
      expect((await completedPayload(pi)).tokens).toEqual({
        input: 100,
        output: 50,
        total: 160,
      })
  ...
  it("reports spend through get_subagent_result too", async () => {
    ...
    const result = await tools
      ...
      .execute("tc-2", { agent_id: "nope" }, undefined, undefined, ctx())
