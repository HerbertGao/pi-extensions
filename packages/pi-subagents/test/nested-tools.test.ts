...
describe("child-safe nested Agent tools", () => {
  ...
  it("resolves nested types without touching the process-global registry", async () => {
    ...
    writeFileSync(
      join(otherAgentDir, "branch-only.md"),
  ...
  it("supports background launches and ownership-scopes result, resume, and steer", async () => {
    ...
    expect((await execute(getResult, { agent_id: "foreign" })).isError).toBe(
      true,
    )
    expect(
      (await execute(steer, { agent_id: "foreign", message: "stop" })).isError,
    ).toBe(true)
    expect(
      (
        await execute(agent, {
  ...
  it("still rejects unknown types when the project configures a fallback", async () => {
    ...
    try {
      ...
      expect(result.content[0].text).toContain(
        "Unknown or disabled nested agent type",
      )
      expect(spawnAndWait).not.toHaveBeenCalled()
    } finally {
  ...
  it("flags a truncated child run instead of passing partial output off as complete", async () => {
    ...
    expect(result.content[0].text).toContain(
      "everything the agent produced is above",
    )
  ...
  it("attributes a nested child's token spend to the owning parent", async () => {
    ...
    expect(parent.lifetimeUsage).toEqual({
      input: 100,
      output: 20,
      cacheWrite: 5,
    })
  ...
  it("files a nested transcript under the root session, honoring output_transcript", async () => {
    records.set("parent-1", {
      id: "parent-1",
      status: "running",
      rootSessionId: "root-session",
    })
    spawnAndWait.mockImplementation(
      async (_pi, _ctx, type, _prompt, options, onSpawned) => {
        const record = {
          id: "child-1",
          type,
          status: "completed",
    ...
    try {
      ...
      expect(records.get("child-1").outputFile).toContain(
        join("root-session", "tasks", "child-1.output"),
      )
