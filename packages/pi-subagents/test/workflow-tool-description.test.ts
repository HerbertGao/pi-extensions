  ...
  ...new Set([
    ...[
      ...workerSource.matchAll(
        /opts\.resume and opts\.(\w+) are mutually exclusive/g,
      ),
    ].map((m) => m[1]),
    ...(workerSource.includes("opts.gate cannot be combined with opts.resume")
      ? ["gate"]
      : []),
  ]),
]
...
function documentedAgentOptions(): {
  keys: Set<string>
  efforts: string[]
  line: number
}[] {
...
describe("the agent() contract it documents", () => {
  it("names every option the runtime accepts", () => {
    for (const option of AGENT_OPTIONS) {
      expect(description, `agent() opts.${option} is undocumented`).toContain(
        option,
      )
    }
  })
  ...
  it("documents no option the runtime would reject by name", () => {
    ...
    expect(
      [...new Set(named)].filter((name) => !AGENT_OPTIONS.includes(name)),
    ).toEqual([])
  })
  ...
  it("lists every option resume cannot be combined with", () => {
    ...
    for (const excluded of RESUME_EXCLUSIONS) {
      expect(
        sentence,
        `resume/${excluded} exclusion missing from the resume line`,
      ).toContain(excluded)
    }
...
describe("the examples it ships", () => {
  ...
  it("uses only options the runtime accepts", () => {
    for (const { keys, line } of documentedAgentOptions()) {
      ...
      expect(
        unknown,
        `unknown agent() option at description line ${line}`,
      ).toEqual([])
    }
  })
  ...
  it("never combines resume with an option that throws", () => {
    ...
    for (const { keys, line } of documentedAgentOptions()) {
      ...
      expect(
        clashes,
        `illegal resume combination at description line ${line}`,
      ).toEqual([])
    }
  })
  ...
  it("never names an effort level the runtime would reject", () => {
    for (const { efforts, line } of documentedAgentOptions()) {
      for (const effort of efforts) {
        expect(
          EFFORT_LEVELS,
          `bad effort at description line ${line}`,
        ).toContain(effort)
      }
    }
  })
