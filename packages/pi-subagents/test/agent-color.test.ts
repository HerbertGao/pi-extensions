...
describe("renderAgentNameLabel", () => {
  it("renders a padded truecolor badge with readable foreground", () => {
    ...
    const badge = renderAgentNameLabel("Code Reviewer", "purple", ansiTheme, {
      bold: true,
    })
    ...
    expect(renderAgentNameLabel("Tester", "navy", theme)).toContain(
      "\u001b[38;2;255;255;255m",
    )
  })
  ...
  it("resets the background when the caller paints none", () => {
    expect(renderAgentNameLabel("Reviewer", "purple", theme)).toMatch(
      /\u001b\[39m\u001b\[49m$/,
    )
  })
  ...
  it("preserves existing theme styling without a valid color", () => {
    expect(
      renderAgentNameLabel("Agent", undefined, theme, {
        fallbackColor: "toolTitle",
        bold: true,
      }),
    ).toBe("<toolTitle>*Agent*</toolTitle>")
    expect(
      renderAgentNameLabel("Agent", "invalid", theme, {
        fallbackColor: "muted",
      }),
