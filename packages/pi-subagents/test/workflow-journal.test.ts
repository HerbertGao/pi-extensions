...
interface Stub {
  ...
  host: {
    spawnAgent: (r: WorkflowSpawnRequest) => Promise<WorkflowSpawnResult>
    abortAgent: () => void
  }
}
  ...
  return {
    ...
    host: {
      async spawnAgent(request) {
        ...
        return reply
          ? reply(request)
          : { ok: true, text: `live:${request.prompt}` }
      },
      abortAgent() {},
    },
  }
...
function recorder() {
  ...
  return {
    entries,
    append: (entry: WorkflowJournalEntry) => entries.push(entry),
  }
}
...
describe("journalKey", () => {
  ...
  it("ignores which phase the row is filed under", () => {
    ...
    expect(journalKey({ prompt: "a", label: "x" })).toBe(
      journalKey({ prompt: "a", label: "x" }),
    )
  })
})
...
describe("journal files", () => {
  ...
  it("keeps the entries before a truncated final line", () => {
    ...
    writeFileSync(
      path,
      `${readFileSync(path, "utf-8")}{"index":1,"key":"k1"`,
      "utf-8",
    )

    expect(readJournal(path)).toEqual([
      { index: 0, key: "k0", ok: true, text: "kept" },
    ])
  })
...
describe("replay", () => {
  ...
  it("records every settled call, so a first run can be resumed", async () => {
    ...
    expect(journal.entries).toEqual([
      ...
      {
        index: 0,
        key: journalKey({ prompt: "first" }),
        ok: true,
        text: "live:first",
      },
      {
        index: 1,
        key: journalKey({ prompt: "second" }),
        ok: true,
        text: "live:second",
      },
    ])
  ...
  it("replays an identical run without spawning anything", async () => {
    ...
    const result = await run(twoAgents, {
      host: second.host,
    ...
    expect(
      second.calls,
      "a full cache hit must not reach the manager at all",
    ).toHaveLength(0)
  })
  ...
  it("re-records replayed calls, so a resume can itself be resumed", async () => {
    ...
    await run(twoAgents, {
      host: stubHost().host,
      journal: { entries: first.entries, append: second.append },
    })
