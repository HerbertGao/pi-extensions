...
function harness(
  ...
  let inputHandler:
    ...
    | undefined
  let widgetFactory:
    | ((tui: any, theme: any) => { render(w: number): string[] })
    | undefined
  let editorText = ""
  let opened = false
  let closed = false
  ...
  return {
    fleet,
    setWorkflows: (list: FleetWorkflow[]) => {
      workflows = list
      fleet.update()
    },
    openedWorkflows: () => openedWorkflows,
    closeWorkflowDialog: async () => {
      closeWorkflowDialog?.()
    ...
    render: (width = 120) =>
      widgetFactory ? widgetFactory(fakeTui, theme).render(width) : [],
    setEditorText: (t) => {
      editorText = t
    },
  ...
  it("re-arms the refresh timer when the list is re-shown (toggle off→on)", () => {
    ...
    try {
      ...
      fleet.setUICtx({
        ...
        notify: () => {},
        custom: (() =>
          new Promise<undefined>(() => {})) as FleetUICtx["custom"],
      })
...
describe("FleetList workflow rows", () => {
  ...
  it("lists a run above the agents, with its counts and stats", () => {
    ...
    expect(rows.findIndex((row) => row.includes("audit-src"))).toBeLessThan(
      agent,
    )
  })
  ...
  it("moves into the list when the only row is a run", () => {
    ...
    expect(
      h.press(DOWN)?.consume,
      "↓ at an empty prompt has somewhere to go",
    ).toBe(true)
    h.press(DOWN)
    h.press(ENTER)
  ...
  it("keeps its hands off the keyboard while the inspector is up", () => {
    ...
    expect(
      h.press(DOWN)?.consume,
      "the dialog's keys are not the list's",
    ).toBeFalsy()
  ...
  it("comes back to the same run when the inspector closes", async () => {
    ...
    expect(
      h.render().find((l) => l.includes("started-meanwhile")),
    ).not.toContain("●")
  })
