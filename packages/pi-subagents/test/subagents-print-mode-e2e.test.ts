...
describe.skipIf(LIVE)(
  "subagents print-mode e2e (scripted faux, real pi-mono)",
  () => {
    ...
    it("records the model and thinking level the child session actually resolved", async () => {
      ...
      const id = /Agent ID: (\S+)/.exec(
        agentToolResults(run.parentSession).join("\n"),
      )?.[1]
    ...
    it("errors clearly when faux mode is given no script", async () => {
      await expect(runPrintMode({ prompt: "x" })).rejects.toThrow(
        /provide `respond` or `steps`/,
      )
    })
...
describe.runIf(LIVE)("subagents print-mode e2e (live LLM, opt-in)", () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(
        calls.some(
          (c) => String(c.subagent_type ?? "").toLowerCase() === "explore",
        ),
      ).toBe(true)
