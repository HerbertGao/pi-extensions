...
const compile = (schema: unknown) => {
  ...
  if (!result.ok)
    throw new Error(`expected a compiled schema, got: ${result.message}`)
...
describe("compiling a script-supplied schema", () => {
  ...
  it("refuses a schema too large to be worth sending", () => {
    const huge = {
      type: "object",
      properties: { a: { type: "string", description: "x".repeat(70_000) } },
    }
...
describe("checking a payload", () => {
  it("passes a conforming value", () => {
    expect(
      compile(FINDINGS).check({ findings: [{ file: "a.ts", line: 3 }] }),
    ).toBe(true)
  })
