...
function card(
  over: Partial<WorkflowCardInput> & { progress: readonly WorkflowEntry[] },
): string[] {
...
const treeRows = (lines: string[]) =>
  lines.filter((l) => /[╭╰├└,`|]/.test(l)).map((l) => l.slice(2))
...
describe("inline glyph mapping", () => {
  ...
  it("says a replayed agent came from the resume journal", () => {
    ...
    expect(row.indexOf("from resume journal")).toBeLessThan(
      row.indexOf("general-purpose"),
    )
  })
  ...
  it("colours done success, error error, and leaves a running row at the terminal default", () => {
    ...
    expect(styled.find((l) => l.includes("done"))).toContain(
      "<success>✔</success>",
    )
    expect(styled.find((l) => l.includes("failed"))).toContain(
      "<error>✘</error>",
    )
...
describe("tree branches", () => {
  ...
  it("still draws a lone group as a closed box", () => {
    const rows = treeRows(
      card({
        progress: [],
        meta: { name: "n", description: "d", phases: [{ title: "Only" }] },
      }),
    )
    ...
    const rows = treeRows(
      card({
        progress: [
          ...
          {
            ...agentEntry({ index: 1, label: "solo2" }),
...
describe("effective-vs-requested disclosure", () => {
  it("names the model alone when the request was honoured", () => {
    expect(formatModel(agentEntry({ index: 0, model: "haiku 4.5" }))).toBe(
      "haiku 4.5",
    )
  })
  ...
  it("discloses a model an agent file pinned over the call's", () => {
    expect(
      formatModel(
        agentEntry({ index: 0, model: "haiku 4.5", requestedModel: "opus" }),
      ),
  ...
  it("discloses a thinking level pi clamped", () => {
    expect(
      formatThinking(
        agentEntry({ index: 0, thinking: "low", requestedThinking: "max" }),
      ),
  ...
  it("renders the level alone when it was honoured", () => {
    expect(
      formatThinking(
        agentEntry({ index: 0, thinking: "high", requestedThinking: "high" }),
      ),
    ).toBe("thinking: high")
  })
...
describe("stat segments", () => {
  ...
  it("omits every stat that is absent", () => {
    ...
    expect(agentStatSegments(agentEntry({ index: 0, toolCalls: 1 }))).toEqual([
      "1 tool call",
    ])
  })
  ...
  it("merges the fallback model into the model segment", () => {
    expect(
      agentStatSegments(
        agentEntry({ index: 0, model: "haiku", fallbackModel: "sonnet" }),
      ),
    ).toEqual(["haiku→sonnet"])
    expect(
      agentStatSegments(
        agentEntry({ index: 0, model: "haiku", fallbackModel: "haiku" }),
      ),
    ).toEqual(["haiku"])
    expect(
      agentStatSegments(agentEntry({ index: 0, fallbackModel: "sonnet" })),
    ).toEqual(["sonnet"])
  })
...
describe("header", () => {
  ...
  it("appends the terminal suffix for each stopped status", () => {
    ...
    for (const [status, suffix] of suffixes) {
      const [header] = card({
        progress: sevenAgents,
        task: { status, startTime: START },
      })
...
describe("width", () => {
  const wide: WorkflowEntry[] = [
    ...
    agentEntry({
      index: 0,
      label:
        "an-extremely-long-agent-label-that-would-otherwise-wrap-the-whole-card",
      agentType: "general-purpose",
    ...
    }),
    {
      type: "workflow_log",
      message:
        "a log line long enough to overflow a narrow terminal on its own",
    },
  ]
  ...
  for (const width of [20, 40, 80]) {
    it(`never exceeds ${width} columns`, () => {
      const lines = card({
        progress: wide,
        width,
        task: { status: "running", startTime: START },
      })
  ...
  it("keeps the header stats when the name has to be cut", () => {
    const lines = card({
      ...
      task: {
        status: "running",
        workflowName: "a-workflow-name-far-too-long-to-fit",
        startTime: START,
      },
    })
...
describe("size warning", () => {
  it("appears once the scheduled agents pass the cap, with Claude Code's wording", () => {
    ...
    expect(lines.at(-1)).toBe(
      "  ⚠ Large workflow · /agents → Workflows to stop",
    )
  })
