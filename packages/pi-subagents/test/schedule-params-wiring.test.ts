...
async function scheduleAndReadBack(
  ...
  expect(jobs, `expected one persisted job, reply was: ${reply}`).toHaveLength(
    1,
  )
...
describe("Agent tool → schedule restrictions", () => {
  ...
  async function scheduleCall(
    ...
    let jobCount = 0
    try {
      jobCount = new ScheduleStore(resolveStorePath(c.cwd, SESSION_ID)).list()
        .length
    } catch {
  ...
  it("refuses `schedule` with `resume` — schedules create fresh agents", async () => {
    const { reply, jobCount, restore } = await scheduleCall({
      resume: "agent-123",
    })
    try {
      expect(reply).toBe(
