...
export function agentCall(
  ...
  return fauxToolCall(
    "Agent",
    { subagent_type: "general-purpose", ...args },
    opts,
  )
}
...
function resolveReply(
  ...
  return typeof reply === "function"
    ? (reply as (c: Context) => FauxReply)(ctx)
    : reply
}
...
export function routeBySession(routes: {
  ...
  return (context) => {
    ...
    const spawned = context.messages.some(
      (m) =>
        m.role === "toolResult" &&
        (m as { toolName?: string }).toolName === "Agent",
    )
    if (spawned) {
...
function isLive(options: RunPrintModeOptions): boolean {
  return (
    ...
    /^(1|true|yes)$/i.test(process.env.PI_E2E_LIVE ?? "")
  )
}
...
export async function runPrintMode(
  ...
  let modelRuntime: FauxProviderRegistration["modelRuntime"] | undefined
  if (live) {
    ...
    if (provider && modelId) {
      ...
      model = (getModel as (p: string, m: string) => Model<string> | undefined)(
        provider,
        modelId,
      )
      if (!model) {
    ...
    if (options.steps) {
      ...
      if (!respond) {
        throw new Error(
          "runPrintMode (faux mode): provide `respond` or `steps`",
        )
      }
  ...
  const { session } = await createAgentSession({
    ...
    agentDir,
    model,
    // Explicit faux runtime in test mode; undefined uses Pi's real auth runtime.
    modelRuntime,
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      const stillRunning = manager?.hasRunning()
        ? " (background subagents still running)"
        : ""
      reject(
        new Error(
          `print-mode runner timed out after ${timeoutMs}ms${stillRunning}`,
        ),
      )
    }, timeoutMs)
  })
        ...
        if (hold) {
          while (!failed && manager?.hasRunning()) {
            ...
            await session.prompt(
              "Background agents have completed. Process their results.",
            )
          }
        }
      })(),
