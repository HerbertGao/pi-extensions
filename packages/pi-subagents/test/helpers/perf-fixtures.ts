...
export function makeFleet(
  ...
  for (let n = 0; n < queued; n++)
    out.push(makeRecord(i++, { status: "queued" }))
  for (let n = 0; n < finished; n++)
    out.push(makeRecord(i++, { status: "completed", completedAt: NOW }))
  return out
}

/** Live per-agent activity, keyed by id, as the widget and fleet list expect. */
export function makeActivity(
  records: { id: string; toolUses: number }[],
): Map<string, any> {
  return new Map(
    records.map((r) => [
      r.id,
...
export function makeManager(records: unknown[]) {
  return {
    ...
    listTombstones: () => [],
    getRecord: (id: string) =>
      records.find((r) => (r as { id: string }).id === id),
    getMaxConcurrent: () => 4,
  } as any
}
...
export function mountFleet(FleetList: any, records: unknown[]) {
  ...
  return {
    ...
    render: (width = 120): string[] =>
      factory ? factory(tui, perfTheme).render(width) : [],
    update: () => fleet.update(),
