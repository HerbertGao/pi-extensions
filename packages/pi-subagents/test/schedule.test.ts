...
describe("SubagentScheduler — static format parsers", () => {
  ...
  it("validateCronExpression rejects non-6-field expressions", () => {
    expect(SubagentScheduler.validateCronExpression("* * * * *").valid).toBe(
      false,
    ) // 5 fields
    expect(SubagentScheduler.validateCronExpression("0 0 9 * * 1").valid).toBe(
      true,
    )
    expect(SubagentScheduler.validateCronExpression("0 0 9 * * *").valid).toBe(
      true,
    )
    expect(SubagentScheduler.validateCronExpression("not-a-cron").valid).toBe(
      false,
    )
  })
  ...
  it("detectSchedule tags type and normalizes input", () => {
    ...
    expect(() => SubagentScheduler.detectSchedule("garbage")).toThrow(
      /Invalid schedule/,
    )
  })
})
...
describe("SubagentScheduler — lifecycle", () => {
  ...
  it("getNextRun uses lastRun when present for interval jobs", () => {
    ...
    const next = scheduler.getNextRun(job.id)
    expect(next).toBe(
      new Date(new Date(lastRun).getTime() + 3_600_000).toISOString(),
    )
  })
...
describe("SubagentScheduler — fire path", () => {
  ...
  describe("infers success vs error from record.status, not promise resolution", () => {
    ...
    it("treats aborted and stopped as errors (terminal failure states)", async () => {
      ...
      expect(scheduler.list().find((j) => j.id === a.id)?.lastStatus).toBe(
        "error",
      )
      expect(scheduler.list().find((j) => j.id === b.id)?.lastStatus).toBe(
        "error",
      )
    })
  })
