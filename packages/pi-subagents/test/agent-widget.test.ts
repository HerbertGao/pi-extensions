...
describe("AgentWidget", () => {
  ...
  it("shows foreground agents in 'all' mode (and by default)", () => {
    ...
    expect(renderLines(manager, "foreground")).toContain(
      "foreground description",
    )
    expect(renderLines(manager, "foreground", () => "all")).toContain(
      "foreground description",
    )
  })
  ...
  it("hides nested children in every coordinator widget mode", () => {
    const manager = {
      listAgents: () => [
        makeRecord("nested", { isBackground: true, parentAgentId: "parent" }),
      ],
    }
  ...
  it("keeps agents with no isBackground flag in 'background' mode", () => {
    ...
    expect(renderLines(manager, "unflagged", () => "background")).toContain(
      "unflagged description",
    )
  })
...
describe("AgentWidget overflow accounting", () => {
  ...
  function renderFleet(counts: {
    ...
    const agents = [
      ...Array.from({ length: counts.running }, (_, i) =>
        record(`run${i}`, "running"),
      ),
      ...Array.from({ length: counts.queued }, (_, i) =>
        record(`q${i}`, "queued"),
      ),
      ...Array.from({ length: counts.finished }, (_, i) =>
        record(`fin${i}`, "completed"),
      ),
    ]
  ...
  for (let running = 0; running <= 8; running++)
    for (let queued = 0; queued <= 8; queued++)
      for (let finished = 0; finished <= 8; finished++)
        SHAPES.push({ running, queued, finished })
  ...
  it("counts everything it hid — the footer total matches what is missing", () => {
    ...
    const shownRunning =
      counts.running -
      [...Array(counts.running).keys()].filter(
        (i) => !body.includes(`run${i} description`),
      ).length
    const shownFinished =
      counts.finished -
      [...Array(counts.finished).keys()].filter(
        (i) => !body.includes(`fin${i} description`),
      ).length
    const actuallyHidden =
      counts.running - shownRunning + (counts.finished - shownFinished)
