...
function commandCtx() {
  ...
  const context = ctx({
    ui: {
      notify: vi.fn((text: string, level?: string) =>
      ...
      custom: vi.fn(
        async (factory: (...args: unknown[]) => unknown, options?: unknown) => {
          const tui = { requestRender: () => {} }
          const theme = {
            fg: (_c: string, t: string) => t,
            bold: (t: string) => t,
          }
          const instance = factory(tui, theme, {}, () => {}) as {
            dispose?: () => void
          }
          built.push(instance)
  ...
  return {
    ...
    built,
    customOptions,
    setPick: (value: string | undefined) => {
      selectPick = value
    },
    offered: () => selectFrom,
...
describe("/agents → Workflows", () => {
  ...
  it("offers the entry in the agents menu", async () => {
    ...
    expect(ui.agentsMenu().some((option) => WORKFLOWS_ENTRY.test(option))).toBe(
      true,
    )
  })
