...
const ALPHA = resolve(
  fileURLToPath(new URL("../fixtures/ext-alpha.mjs", import.meta.url)),
)
/** Registers `beta_tool` — loaded but NOT selected by the `ext:` selector below. */
const BETA = resolve(
  fileURLToPath(new URL("../fixtures/ext-beta.mjs", import.meta.url)),
)
