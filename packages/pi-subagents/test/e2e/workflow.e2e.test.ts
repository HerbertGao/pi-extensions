...
function workflowProject(): string {
  ...
  writeFileSync(
    ...
    JSON.stringify({ workflowsEnabled: true }),
  )
  return dir
}
...
const asText = (context: { messages?: unknown[] }) =>
...
describe("Workflow end to end", () => {
  it("runs a real script whose agents reach real sessions with the script's prompts", async () => {
    ...
    try {
      ...
      expect(
        asText(run.parentSession as unknown as { messages?: unknown[] }),
      ).toContain("Task ID")
      ...
      expect(
        journals,
        "a real run must leave a journal beside its script",
      ).toHaveLength(1)
      const recorded = readJournal(journals[0])
      expect(recorded).toHaveLength(2)
      expect(recorded.every((entry) => entry.ok && entry.key.length > 0)).toBe(
        true,
      )
      ...
      expect(
        childSystemPrompts.every((p) => p.includes("<workflow_child>")),
      ).toBe(true)
      expect(childSystemPrompts[0]).toContain(
  ...
  it("runs a saved workflow by name, with schema and a nested child, end to end", async () => {
    ...
    try {
      ...
      expect(JSON.parse(String(recorded[0].text))).toEqual({
        files: ["a.ts", "b.ts"],
      })
    } finally {
      await run.dispose?.()
...
const entriesFor = (cwd: string) =>
  journalsFor(cwd).flatMap((path) => readJournal(path))
...
describe.runIf(LIVE)("Workflow end to end (live LLM, opt-in)", () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(
        calls.length,
        "the model never called SubagentWorkflow",
      ).toBeGreaterThan(0)
      ...
      expect(
        toolResultsNamed(run.parentSession, "SubagentWorkflow").join("\n"),
      ).toContain("Task ID")
      ...
      expect(
        calls.some(
          (c) =>
            ...
            String(c.script).includes("live-fanout"),
        ),
      ).toBe(true)
      expect(
        toolResultsNamed(run.parentSession, "SubagentWorkflow").join("\n"),
  ...
  it(
    ...
    async () => {
      ...
      expect(
        toolCallsNamed(run.parentSession, "SubagentWorkflow").length,
      ).toBeGreaterThan(0)
      expect(
        toolResultsNamed(run.parentSession, "SubagentWorkflow").join("\n"),
      ).toContain("Task ID")
      ...
      const settled = await waitFor(
        () =>
          entriesFor(cwd).some(
            (entry) => entry.ok && (entry.text ?? "").trim().startsWith("{"),
          ),
        LIVE_SETTLE_TIMEOUT,
      )
      const recorded = entriesFor(cwd)
      ...
      const structured = recorded.find((entry) =>
        (entry.text ?? "").trim().startsWith("{"),
      )
      const payload = JSON.parse(String(structured?.text)) as {
        fruit?: unknown
      }
