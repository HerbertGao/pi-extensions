...
vi.mock("../src/output-file.js", async () => {
  const actual = await vi.importActual<typeof import("../src/output-file.js")>(
    "../src/output-file.js",
  )
  return {
    ...actual,
...
describe("Agent tool — background resume wiring", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(runAgent).mockImplementation(
      async (_ctx, _type, _prompt, options: any) => {
  ...
  it("refuses a second background resume while the first run is in flight", async () => {
    ...
    await tools
      .get("Agent")
      .execute("resume-1", params, undefined, undefined, ctx)
    const second = await tools
      .get("Agent")
      .execute("resume-2", params, undefined, undefined, ctx)
