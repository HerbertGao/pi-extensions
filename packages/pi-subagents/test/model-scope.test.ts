...
describe("checkModelScope", () => {
  ...
  function setEnabledModels(models: string[]) {
    ...
    writeFileSync(
      ...
      JSON.stringify({ enabledModels: models }),
    )
  }

  function check(
    overrides: Partial<Parameters<typeof checkModelScope>[0]> = {},
  ) {
  ...
  describe("out of scope", () => {
    ...
    it("refuses a caller-supplied choice and lists what is allowed", () => {
      const verdict = check({
        callerSupplied: true,
        modelInput: "anthropic/claude-opus-4-6",
      })
    ...
    it("names the resolved model in the warning when there was no raw input", () => {
      ...
      expect((verdict as { message: string }).message).not.toContain(
        "undefined",
      )
    })
