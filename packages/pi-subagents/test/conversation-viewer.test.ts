...
const { ConversationViewer, RESULT_MAX_CHARS } =
  await import("../src/ui/conversation-viewer.js")
...
describe("ConversationViewer invocation line", () => {
  ...
  it("falls back to the short label when no canonical id was captured", () => {
    expect(invocationLine({ modelName: "sonnet 4.6", thinking: "high" })).toBe(
  ...
  it("discloses a model and level the run did not honor", () => {
    expect(
      ...
      "↳ anthropic/claude-haiku-4-5 (asked google/gemini-3-pro) · thinking: low (asked max)",
    )
  })
...
describe("ConversationViewer", () => {
  ...
  describe("Markdown rendering", () => {
    ...
    it("caps a tool result at RESULT_MAX_CHARS, not 500, and says what it dropped", () => {
      ...
      const out = strip(
        viewerFor(result(lines.join("\n")), undefined, undefined, 4000)
          .render(80)
          .join("\n"),
      )
    ...
    it("leaves a result under the cap untouched", () => {
      ...
      const text = `head\n${"filler line\n".repeat(200)}tail`
      const out = strip(
        viewerFor(result(text), undefined, undefined, 600)
          .render(80)
          .join("\n"),
      )
    ...
    it("caps bash output with the same rule as a tool result", () => {
      const messages = [
        { role: "bashExecution", command: "yes", output: "y\n".repeat(20000) },
      ]
    ...
    it("keeps tool results dim even when rendering them as Markdown", () => {
      ...
      const line = (viewer as any)
        .buildContentLines(76)
