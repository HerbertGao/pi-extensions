...
describe("preloadSkills", () => {
  ...
  it("ignores .txt files (only .md is supported)", () => {
    ...
    expect(preloadSkills(["error-handling"], tmpDir)[0].content).toContain(
      "not found",
    )
  })
  ...
  it("ignores extensionless files (only .md is supported)", () => {
    ...
    expect(preloadSkills(["bare-skill"], tmpDir)[0].content).toContain(
      "not found",
    )
  })
  ...
  it("loads a top-level <name>/SKILL.md from project", () => {
    ...
    expect(preloadSkills(["writing-go"], tmpDir)[0].content).toContain(
      "Writing Go",
    )
  })
  ...
  it("loads a top-level <name>/SKILL.md from getAgentDir()/skills", () => {
    ...
    expect(preloadSkills(["writing-python"], tmpDir)[0].content).toContain(
      "Writing Python",
    )
  })
  ...
  it("finds nested <subdir>/<name>/SKILL.md in getAgentDir()/skills", () => {
    writeSkillDir(
      join(globalRoot(), "dev-tools"),
      "using-modern-cli",
      "# Modern CLI",
    )
    expect(preloadSkills(["using-modern-cli"], tmpDir)[0].content).toContain(
      "Modern CLI",
    )
  })
  ...
  it("loads <name>/SKILL.md from project .agents/skills (Agent Skills spec)", () => {
    writeSkillDir(
      join(tmpDir, ".agents", "skills"),
      "writing-rust",
      "# Writing Rust",
    )
    expect(preloadSkills(["writing-rust"], tmpDir)[0].content).toContain(
      "Writing Rust",
    )
  })
  ...
  it("skips node_modules during recursion", () => {
    writeSkillDir(
      join(projectRoot(), "node_modules", "some-pkg"),
      "leaked",
      "should not load",
    )
  ...
  it("skips skill names with spaces", () => {
    expect(preloadSkills(["my skill"], tmpDir)[0].content).toContain(
      "path traversal",
    )
  })
  ...
  it("skips skill names exceeding 128 characters", () => {
    ...
    expect(preloadSkills([longName], tmpDir)[0].content).toContain(
      "path traversal",
    )
  })
