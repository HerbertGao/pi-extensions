...
function report(head, base, ref) {
  ...
  console.log(
    `  ${"-".repeat(width)}  ${"-".repeat(12)}  ${"-".repeat(12)}  ${"-".repeat(9)}`,
  )
  for (const r of rows) {
    const delta =
      r.delta == null
    ...
    console.log(
      `  ${pad(r.name, width)}  ${padStart(fmt(r.base), 12)}  ${padStart(fmt(r.head), 12)}  ${padStart(delta, 9)}`,
    )
  }
...
if (!existsSync(join(REPO, PERF_DIR)))
  usage(`No ${PERF_DIR} directory here — run from the repo root.`)
...
try {
  ...
  for (let round = 1; round <= opts.rounds; round++) {
    ...
    mergeMin(
      head,
      runBench(REPO, opts.filter, join(jsonDir, `head-${round}.json`)),
    )
    process.stdout.write(`${sha}… `)
    mergeMin(
      base,
      runBench(worktree, opts.filter, join(jsonDir, `base-${round}.json`)),
    )
    console.log("done")
  }
