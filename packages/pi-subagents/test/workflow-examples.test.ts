...
const EXAMPLES_DIR = fileURLToPath(
  new URL("../examples/workflows", import.meta.url),
)
...
const readExample = (dir: string, name: string) =>
...
function stubHost(options: { gateFailsFor?: string[] } = {}): {
  ...
  const host: WorkflowHost = {
      ...
      if (request.schema !== undefined) {
        ...
        if (label.startsWith("review:")) {
          return {
            ...
            text: JSON.stringify({
              findings: [
                { title: `${label} finding`, file: "a.ts", severity: "low" },
              ],
            }),
            outputTokens: 10,
          }
        ...
        if (label.startsWith("verify:")) {
          return {
            ok: true,
            text: JSON.stringify({ isReal: true, why: "reproduced" }),
            outputTokens: 10,
          }
        }
        // lib/count-child.js
        if (label === "scan") {
          return {
            ok: true,
            text: JSON.stringify({ files: ["a.ts", "b.ts"] }),
            outputTokens: 10,
          }
        }
    ...
    async resumeAgent(_agentId, prompt) {
      return {
        ok: true,
        text: `resumed:${prompt.slice(0, 20)}`,
        outputTokens: 10,
      }
    },
    ...
    loadWorkflow(ref) {
      ...
      const name = ref.name
      if (name === undefined)
        return { ok: false, message: "only `name` refs are stubbed" }
...
const runExample = (name: string, host: WorkflowHost) =>
  runWorkflow({
    script: readExample(EXAMPLES_DIR, name),
    host,
...
describe("shipped example workflows", () => {
  ...
  describe.each([
    ...exampleFiles.map((n) => [EXAMPLES_DIR, n] as const),
    ...childFiles.map((n) => [LIB_DIR, n] as const),
  ])("%s/%s", (dir, name) => {
  ...
  describe("returned values", () => {
    it("fan-out-audit returns one verified finding per discovered file", async () => {
      ...
      expect(result.value).toEqual([
        "ok:verify:src/routes/a.ts",
        "ok:verify:src/routes/b.ts",
      ])
    })
