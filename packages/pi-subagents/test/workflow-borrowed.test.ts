  ...
  return {
    ...
    host: {
      ...
      async runGate(command, gateOptions) {
        gateCalls.push({
          command,
          agentId: gateOptions.agentId,
          cwd: gateOptions.cwd,
        })
        return options?.gate
          ? await options.gate(command)
          : { ok: true, output: "" }
      },
...
describe("gate", () => {
  it("turns a failing gate into a failed agent, with the command output as the error", async () => {
    ...
    const result = await run(
      'return await agent("fix the auth test", { gate: "npm test" });',
      {
        host,
      },
    )
    ...
    expect(gateCalls).toEqual([
      { command: "npm test", agentId: "wf-agent-0", cwd: undefined },
    ])
  })

  it("leaves a passing gate's agent done, with its value intact", async () => {
    const { host, gateCalls } = stubHost({
      gate: () => ({ ok: true, output: "42 passing" }),
    })

    const result = await run(
      'return await agent("fix it", { gate: "npm test" });',
      { host },
    )
  ...
  it("keeps the agent's own accounting when the gate rejects the work", async () => {
    ...
    const result = await run('return await agent("x", { gate: "npm test" });', {
      host,
    })
    const entry = latest(result.progress, 0)
  ...
  it("names the command when a failing gate says nothing", async () => {
    ...
    const result = await run(
      'return await agent("x", { gate: "npm run lint" });',
      { host },
    )
    expect(latest(result.progress, 0)?.error).toBe(
      "Gate command failed: npm run lint",
    )
  })

  it("does not run for an agent that already failed", async () => {
    const { host, gateCalls } = stubHost({
      reply: () => ({ ok: false, error: "child exploded" }),
    })
  ...
  it("offers the command to the host, and shapes the verdict a host that ran it reports", async () => {
    ...
    const { host, gateCalls, calls } = stubHost({
      reply: () => ({
        ok: true,
        text: "done",
        gate: { ok: false, output: "1 failing" },
      }),
    })
  ...
  it("rejects an empty gate command", async () => {
    const { host, calls } = stubHost()
    const result = await run('return await agent("x", { gate: "  " });', {
      host,
    })
    expect(result.status).toBe("failed")
    expect(result.error).toContain(
      "agent() opts.gate requires a non-empty string.",
    )
...
describe("resume", () => {
  it("continues the child that ran under the label, and still reports progress", async () => {
    ...
    expect(
      agentEntries(result.progress).filter((entry) => entry.index === 1),
    ).toHaveLength(3)
  })
  ...
  it("says so when nothing has completed yet", async () => {
    const { host } = stubHost()
    const result = await run('return await agent("c", { resume: "impl" });', {
      host,
    })
    expect(result.status).toBe("failed")
  ...
  describe("guardrails", () => {
    ...
    it("rejects an empty resume label", async () => {
      const { host } = stubHost()
      const result = await run('return await agent("b", { resume: "" });', {
        host,
      })
      expect(result.status).toBe("failed")
      expect(result.error).toContain(
        "agent() opts.resume requires a non-empty string.",
      )
    })
    ...
    it("rejects a resume with no follow-up task", async () => {
      ...
      expect(result.status).toBe("failed")
      expect(result.error).toContain(
        "agent(prompt) requires a non-empty string.",
      )
      expect(resumeCalls).toEqual([])
    })
  })
...
describe("unawaited launches", () => {
  it("fails the run and aborts the child when a launch is dropped", async () => {
    const { host, aborted } = stubHost({
      reply: () => new Promise<WorkflowSpawnResult>(() => {}),
    })
  ...
  it("does not fire for a script that awaits everything", async () => {
    ...
    expect(result.status).toBe("completed")
    expect(result.value).toEqual(["ok:slow", ["ok:p1", "ok:p2"], ["ok:s1:x"]])
  })
  ...
  it("does not report unawaited launches when the run was aborted", async () => {
    ...
    const promise = run(
      [
        'agent("dropped", { label: "drop" });',
        'await agent("blocks forever");',
        "return null;",
      ].join("\n"),
      {
        host,
        ...
        onProgress(entries) {
          if (
            entries.some(
              (entry) =>
                entry.type === "workflow_agent" && entry.startedAt != null,
            )
          )
            started()
        },
