...
function toolResultTexts(
  ...
  for (const m of context.messages) {
    ...
    const text = ((m.content ?? []) as Array<{ text?: string }>)
      .map((b) => b.text ?? "")
      .join("")
    out.push({ name, text })
  }
  ...
  it("backgrounds a nested child, polls it by id, and streams its transcript", async () => {
    ...
    const respond = (context: Context): FauxReply => {
      ...
      if (text.includes("Delegate this downward")) {
        ...
        if (spawned) {
          ...
          return fauxToolCall("get_subagent_result", {
            agent_id: id,
            wait: true,
          })
        }
      ...
      if (toolResultTexts(context).some((r) => r.name === "Agent"))
        return "parent done"
      return agentCall({
        subagent_type: "orchestrator",
        description: "delegate",
    ...
    try {
      ...
      const orchestratorResult = run.parentSession.messages
        ...
        .flatMap((m) =>
          (m.content as Array<{ text?: string }>).map((b) => b.text ?? ""),
        )
        .join("\n")
      ...
      const transcripts = findOutputFiles(transcriptRoot).map((f) =>
