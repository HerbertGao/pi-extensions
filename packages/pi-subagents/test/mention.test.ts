...
describe("assignHandle", () => {
  ...
  it("keeps counting past every taken form", () => {
    expect(assignHandle("explore", new Set(["explore", "explore-2"]))).toBe(
      "explore-3",
    )
  })
  ...
  it("skips a gap rather than reusing a live handle", () => {
    ...
    expect(assignHandle("explore", new Set(["explore", "explore-3"]))).toBe(
      "explore-2",
    )
  })
})
...
describe("resolveHandleToType", () => {
  ...
  it("round-trips every registered type", () => {
    for (const type of TYPES)
      expect(resolveHandleToType(handleBase(type), TYPES)).toBe(type)
  })
...
describe("describeMention", () => {
  it("uses the message as the agent's short label", () => {
    expect(describeMention("find every retry marker")).toBe(
...
describe("parseMention", () => {
  ...
  it("trims the message and accepts a newline as the separator", () => {
    expect(parseMention("@explore   spaced   ")).toEqual({
      handle: "explore",
      message: "spaced",
    })
