...
function build() {
  ...
  return {
    tool: createStructuredOutputTool(compilation.compiled, capture),
    capture,
  }
}
...
describe("the StructuredOutput tool", () => {
  ...
  it("asks the provider to constrain sampling, but does not require it", () => {
    ...
    expect(build().tool.constrainedSampling).toEqual({
      type: "json_schema",
      strict: "prefer",
    })
  })
  ...
  it("captures a conforming payload as canonical JSON", async () => {
    ...
    expect(JSON.parse(capture.json as string)).toEqual({
      file: "a.ts",
      line: 3,
    })
...
describe("the retry prompt", () => {
  it("distinguishes never answering from answering wrongly", () => {
    ...
    const wrong = {
      called: true,
      lastError: "$: must have required properties file",
    }
  ...
  it("always ends by asking for the call", () => {
    for (const capture of [
      createStructuredCapture(),
      { called: true, lastError: "x" },
    ]) {
      expect(structuredRetryPrompt(capture)).toMatch(
