...
describe("abortable", () => {
  ...
  it("propagates a rejection from the wrapped promise", async () => {
    const controller = new AbortController()
    await expect(
      abortable(Promise.reject(new Error("work failed")), controller.signal),
  ...
  it("removes its abort listener once the promise rejects", async () => {
    ...
    await expect(
      abortable(Promise.reject(new Error("nope")), controller.signal),
    ).rejects.toThrow()
  ...
  it("a late abort after normal settlement is inert", async () => {
    ...
    const unhandled = await withUnhandledRejectionSpy(async () => {
      const controller = new AbortController()
      await expect(
        abortable(Promise.resolve("done"), controller.signal),
      ).resolves.toBe("done")
