...
it("loads the package entrypoint and executes fused tools in an all-enabled Pi session", async () => {
	...
	try {
		...
		const resourceLoader = new DefaultResourceLoader({
			...
			agentDir,
			settingsManager,
			additionalExtensionPaths: [fileURLToPath(new URL("../src/sol-pi/index.ts", import.meta.url))],
			extensionFactories: [
				{
					name: "package-test-provider",
		...
		const solPi = resourceLoader
			.getExtensions()
			.extensions.find((extension) => extension.path.endsWith("src/sol-pi/index.ts"));
