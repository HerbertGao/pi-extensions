...
import { hasRunningSubagents } from "../src/sol-pi/extensions/online-context-compact/extension.ts";
...
describe("Online Context Compact extension", () => {
	it("registers one tool and only public Pi lifecycle hooks", () => {
		...
		expect([...pi.handlers.keys()].toSorted()).toEqual([
			"agent_settled",
	...
	it("treats a running subagent registry as a compaction guard", () => {
		const key = Symbol.for("pi-subagents:manager");
		const globals = globalThis as Record<PropertyKey, unknown>;
		const previous = globals[key];
		try {
			globals[key] = { hasRunning: () => true };
			expect(hasRunningSubagents()).toBe(true);
