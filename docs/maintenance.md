...
The editable source is `.github/workflows/upstream-upgrade.md`; the lock file and `.github/aw/actions-lock.json` are generated by GitHub Agentic Workflows `v0.89.3` and must not be hand-edited. The workflow runs only stable `gemini-3.8-flash` against a separate Google API allowance, with model fallback disabled, an 80 AIC per-run cap, and a 100-turn limit. It treats monitor findings as candidates, opens at most one draft PR, restricts changed paths, runs `bun run check`, and comments on the tracking Issue with `@HerbertGao` when a repository-level decision blocks a coherent update. It cannot merge, publish, release, tag, or modify workflow files. Because gh-aw `v0.89.3` cannot use Gemini for threat detection, that feature is disabled instead of falling back to Copilot; draft-only output, path restrictions, and maintainer review remain the compensating controls. A pre-agent step also sets the ephemeral runner's Gemini auth type to `gemini-api-key`: Gemini CLI `0.59.0` otherwise rejects the AWF gateway before the proxy can handle the isolated key. The real key remains in the AWF sidecar and is still excluded from the agent environment; remove this workaround when the upstream CLI/compiler fix lands. A separate setup step copies pinned Bun 1.3.14 into `$RUNNER_TOOL_CACHE/gh-aw-bun/bin` because `setup-bun`'s default install path is not mounted into the AWF sandbox. API-proxy token steering is disabled so the request remains on `gemini-3.8-flash`; the workflow also allowlists that model and supplies its Google pricing because the pinned AWF catalog predates 3.8. The workflow gives shell tools a 10-minute timeout and treats network/cache failures as an immediate incomplete result rather than retrying into the AIC budget. Runtime `.gemini/` settings are excluded from formatting checks.

This personal repository needs two Actions secrets before the workflow can run:

- `GEMINI_API_KEY`: a Google AI Studio API key restricted to the Gemini API; inference is billed against the associated Google project rather than the Copilot allowance;
- `GH_AW_CI_TRIGGER_TOKEN`: a fine-grained PAT with repository **Contents: Read and write**, used only for the extra empty commit that makes the agent-created PR trigger CI.

`COPILOT_GITHUB_TOKEN` is not used for this workflow's inference after the Gemini migration.
...
```bash
gh extension install github/gh-aw --pin v0.89.3
gh aw compile upstream-upgrade --strict --gh-aw-ref v0.89.3 --validate --actionlint
...
The released range `5d7a266..9f20006` was reviewed commit by commit:

- `b8b2d89` and its `3b2d49c` follow-up are **ported** into the local rich-diff parser: Pi's space-delimited numbered rows are accepted only while no hunk header has been seen, so unified hunks keep numeric source content instead of treating it as a gutter; blank-number `...` omission rows are metadata and no longer acquire invented line numbers or advance source cursors. Focused real-producer, unified-hunk, omission, and literal-ellipsis regressions are included.
- `26aa98d` is **selectively ported**: long path summaries are relativized against the tool cwd and middle-truncated so the filename stays visible in single-tool, grouping, and compact edit/write summaries. The local implementation keeps live-viewport clipping instead of upstream's fixed `inputClip` ceiling.
- `2299ac9` merges the diff-parser pull request without independent changes, and `9f20006` is release metadata represented through local provenance.

The Fork continues to preserve its renderer lifecycle, malformed-agent recovery, Markdown fence protection, terminal-width behavior, and rich-diff/write-state correctness tests. Package provenance advances to released `v0.8.70` at `9f20006`; the repository review cursor advances to the same commit.
...
- `pi-jev-auto-mode@0.5.0` pins the companion release.
- `pi-multi-account@1.23.2` pins the companion release.
- `@herbertgao/pi-cc-extensions` upstream updates `0.9.3` - `0.9.5` and unreleased repository commits are deferred as there is no offline review text or diff available, and the sandbox has no outbound internet access to review them.
...
- `@narumitw/pi-btw@0.61.0` pins the companion release.
- `pi-typesafe@0.7.1` pins the companion release.
- `NVlabs SoL-Pi` unreleased repository commits are deferred as there is no new published package release.
